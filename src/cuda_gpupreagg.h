...
/*
 * nogroup merge functions - to be generated by PG-Strom on the fly
 *
 * 
 *
 *
...
AGGCALC_INIT_TEMPLATE(min_int, INT_MAX)
AGGCALC_INIT_TEMPLATE(max_int, INT_MIN)
AGGCALC_INIT_TEMPLATE(add_int,  0)
AGGCALC_INIT_TEMPLATE(min_long, LONG_MAX)
AGGCALC_INIT_TEMPLATE(max_long, LONG_MIN)
...
#define AGGCALC_SHUFFLE_TEMPLATE(name)									\
	...
	{																	\
		...
		newval_dclass = __shfl_sync(__activemask(), src_dclass, lane_id); \
		newval_datum  = __shfl_sync(__activemask(), src_datum,  lane_id); \
