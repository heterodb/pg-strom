								 ...
								 kern_global_hashslot *f_hash)
{
	kern_parambuf *kparams = KERN_GPUPREAGG_PARAMBUF(kgpreagg);
	DECL_KERNEL_CONTEXT(u);
	__shared__ cl_char	l_dclass[GPUPREAGG_NUM_ACCUM_VALUES * MAXWARPS_PER_BLOCK];
	__shared__ Datum	l_values[GPUPREAGG_NUM_ACCUM_VALUES * MAXWARPS_PER_BLOCK];
	cl_char				p_dclass[GPUPREAGG_NUM_ACCUM_VALUES];
	Datum				p_values[GPUPREAGG_NUM_ACCUM_VALUES];
	 ...
	 */
	assert(GPUPREAGG_NUM_ACCUM_VALUES == kds_slot->ncols &&
		   GPUPREAGG_NUM_ACCUM_VALUES == kds_final->ncols);
