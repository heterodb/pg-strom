 ...
 *      in at runtime.
 *
 * static function at createplan.c
 */
List *
order_qual_clauses(PlannerInfo *root, List *clauses)
{
	...
	int			i, nitems = list_length(clauses);
	QualItem   *items;
	ListCell   *lc;
	List	   *result;
	...
	foreach(lc, clauses)
	{
		...
		{
			/*
			 * MEMO: Here is PG-Strom specific heuristics. Device functions
			 * that take variable length datum is usually expensive than
			 * simple functions that handle only fixed-length datum.
			 * So, we adjust the cost according to the number of varlena.
			 */
			RestrictInfo *rinfo = (RestrictInfo *) clause;
			List	   *vars_list;
			ListCell   *cell;

			vars_list = pull_vars_of_level((Node *)rinfo->clause, 0);
			foreach (cell, vars_list)
			{
				...
				if (IsA(node, Var))
				{
					typlen = get_typlen(((Var *)node)->vartype);
					if (typlen < 0)
						qcost.per_tuple += 1.0;
				}
			}
