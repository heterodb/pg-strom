...
typedef struct
{
	CUcontext		cuda_context; /* copy from gpuContext that owns this pool */
 ...
 * variables
 */
static __thread CUevent			__MY_EVENTS_PER_THREAD_ARRAY[HETERODB_EXTRA_MAX_GPUS];
...
#define MY_EVENT_PER_THREAD		(__MY_EVENTS_PER_THREAD_ARRAY[MY_DINDEX_PER_THREAD])
...
__gpuMemAllocNewSegment(gpuMemoryPool *pool, size_t segment_sz)
{
	...
	rc = cuCtxPushCurrent(pool->cuda_context);
	if (rc != CUDA_SUCCESS)
				  ...
				  size_t dev_total_memsz)
{
	...
	pool->cuda_context = gcontext->cuda_context;
...
__putGpuQueryBufferNoLock(gpuQueryBuffer *gq_buf)
{
	...
	if (--gq_buf->refcnt == 0)
	{
		...
		if (gq_buf->m_kmrels)
		{
			kern_multirels *m_kmrels = (kern_multirels *)gq_buf->m_kmrels;
			...
			for (int depth=1; depth <= m_kmrels->num_rels; depth++)
			{
				...
				if (kbuf_parts)
				{
					...
					{
						CUdeviceptr	m_kds_in = kbuf_parts->parts[k].kds_devptr;

						if (m_kds_in != 0UL)
						{
							rc = cuMemFree(m_kds_in);
							if (rc != CUDA_SUCCESS)
								__gsDebug("failed on cuMemFree: %s",
										  cuStrError(rc));
						}
					}
				}
		...
		{
			CUdeviceptr	m_kds_final = gq_buf->gpus[__dindex].m_kds_final;
			CUdeviceptr	m_kds_fallback = gq_buf->gpus[__dindex].m_kds_fallback;

			if (m_kds_final)
			{
				rc = cuMemFree(m_kds_final);
				if (rc != CUDA_SUCCESS)
					__gsDebug("failed on cuMemFree: %s", cuStrError(rc));
			}
			if (m_kds_fallback)
			{
				rc = cuMemFree(m_kds_fallback);
				if (rc != CUDA_SUCCESS)
					__gsDebug("failed on cuMemFree: %s", cuStrError(rc));
			}
		}
								   ...
								   gpuQueryBuffer *gq_buf)
{
	...
	if (has_gist)
	{
		...
		if (rc != CUDA_SUCCESS)
		{
			gpuClientELog(gclient, "failed on cuEventRecord: %s",
						  cuStrError(rc));
			return false;
		}
		...
		if (rc != CUDA_SUCCESS)
		{
			gpuClientELog(gclient, "failed on cuEventSynchronize: %s",
						  cuStrError(rc));
			return false;
		}
	}
									  ...
									  int hash_remainder)
{
	...
	rc = cuMemAllocManaged(&m_kds_dst, kds_length,
						   CU_MEM_ATTACH_GLOBAL);
	if (rc != CUDA_SUCCESS)
	{
										   ...
										   kern_buffer_partitions *kbuf_parts)
{
	...
	gpuContext **part_gcontexts = alloca(sizeof(gpuContext *) * hash_divisor);
	dlist_iter	iter;
	...
	for (int k=0; k < hash_divisor; k++)
	{
		...
		double		part_score = -1.0;	/* randomizatino */

		dlist_foreach (iter, &gpuserv_gpucontext_list)
		{
			gpuContext *__gcontext = dlist_container(gpuContext,
													 chain, iter.cur);
			double		__score = drand48();

			if ((part_mask & __gcontext->cuda_dmask) == 0)
				continue;
			if (!gcontext ||
				/* optimal GPUs are preferable than others */
				((gclient->optimal_gpus & gcontext->cuda_dmask)   == 0 &&
				 (gclient->optimal_gpus & __gcontext->cuda_dmask) != 0) ||
				/* both GPUs are optimal and higher randomization score */
				((gclient->optimal_gpus & gcontext->cuda_dmask)   != 0 &&
				 (gclient->optimal_gpus & __gcontext->cuda_dmask) != 0 &&
				 __score > part_score) ||
				/* both GPUs are non-optimal and higher randomization score */
				((gclient->optimal_gpus & gcontext->cuda_dmask)   == 0 &&
				 (gclient->optimal_gpus & __gcontext->cuda_dmask) == 0 &&
				 __score > part_score))
			{
	...
	for (int k=0; k < hash_divisor; k++)
	{
		...
		m_kds_in = __setupGpuJoinPinnedInnerBufferCommon(gclient,
														 gcontext,
														 gq_src,
														 hash_divisor,
														 k);
		if (m_kds_in == 0UL)
			goto error;
		kbuf_parts->parts[k].kds_devptr = m_kds_in;
	}
	return true;
		...
		if (m_kds_in != 0UL)
		{
			...
			if (rc != CUDA_SUCCESS)
				__FATAL("failed on cuMemFree(%p): %s",
						(void *)m_kds_in, cuStrError(rc));
		}
		kbuf_parts->parts[k].kds_devptr = 0UL;
	}
	if (gq_src)
...
static bool
__setupGpuJoinPinnedInnerBufferSingle(gpuClient *gclient,
									  gpuQueryBuffer *gq_buf,
									  kern_multirels *m_kmrels,
									  int depth)
{
	...
	/*
	 * Even if non-partitioned inner buffer case, we have to
	 * consolidate the buffer if multiple source buffers are
	...
	else if (kds_nchunks == 1)
	{
		...
		m_kmrels->chunks[depth-1].kds_devptr = (CUdeviceptr)kds_src;
	}
	else
	{
		/*
		 * Elsewhere, reconstruct the pinned-inner buffers
		 */
		gpuContext *gcontext = NULL;
		...
		dlist_foreach (iter, &gpuserv_gpucontext_list)
		{
			...
			if ((gclient->optimal_gpus & __gcontext->cuda_dmask) != 0)
			{
				...
				if (!gcontext || rand_score < __score)
				{
					gcontext = __gcontext;
					rand_score = __score;
				}
		...
		if (m_kds_in == 0UL)
			goto error;
		m_kmrels->chunks[depth-1].kds_devptr = m_kds_in;
		putGpuQueryBuffer(gq_src);
	}
	return true;
							   ...
							   uint32_t kmrels_handle)
{
	...
	for (int depth=1; depth <= m_kmrels->num_rels; depth++)
	{
		...
		if (m_kmrels->chunks[depth-1].pinned_buffer)
		{
			...
			{
				if (!__setupGpuJoinPinnedInnerBufferSingle(gclient,
														   gq_buf,
														   m_kmrels,
														   depth))
					goto error;
			...
			fprintf(stderr, "pinned inner buffer [%d] reconstruction: %.3fms\n",
					depth,
							 ...
							 kern_data_store *kds_final_head)
{
	...
	dlist_foreach (iter, &gpuserv_gpucontext_list)
	{
		...
		if (rc != CUDA_SUCCESS)
		{
			...
			for (__dindex=0; __dindex < numGpuDevAttrs; __dindex++)
			{
				m_kds_final = gq_buf->gpus[__dindex].m_kds_final;
				if (m_kds_final)
					cuMemFree(m_kds_final);
			}
			return false;
		}
						 ...
						 XpuCommand *xcmd)
{
	...
	if (gclient->__session[MY_DINDEX_PER_THREAD])
		session = (kern_session_info *)
			gclient->__session[MY_DINDEX_PER_THREAD]->m_devptr;
...
gpuservGpuCacheManager(void *__arg)
{
	...
	memset(__MY_EVENTS_PER_THREAD_ARRAY, 0, sizeof(__MY_EVENTS_PER_THREAD_ARRAY));
	pg_memory_barrier();
...
gpuservGpuWorkerMain(void *__arg)
{
	...
	/* per-thread / per-device event object */
	memset(__MY_EVENTS_PER_THREAD_ARRAY, 0, sizeof(__MY_EVENTS_PER_THREAD_ARRAY));
	dlist_foreach (iter, &gpuserv_gpucontext_list)
	{
		gpuContext *__gcontext = dlist_container(gpuContext, chain, iter.cur);

		rc = cuCtxSetCurrent(__gcontext->cuda_context);
		if (rc != CUDA_SUCCESS)
			__FATAL("failed on cuCtxSetCurrent: %s", cuStrError(rc));
		rc = cuEventCreate(&cuda_event, CU_EVENT_BLOCKING_SYNC);
		if (rc != CUDA_SUCCESS)
			__FATAL("failed on cuEventCreate: %s", cuStrError(rc));
		__MY_EVENTS_PER_THREAD_ARRAY[__gcontext->cuda_dindex] = cuda_event;
	}
	...
	for (int __dindex=0; __dindex < numGpuDevAttrs; __dindex++)
	{
		cuda_event = __MY_EVENTS_PER_THREAD_ARRAY[__dindex];
		if (cuda_event)
