					  ...
					  int outer_depth)
{
	...
	row_inval_sz = STROMALIGN(offsetof(gpupreaggRowInvalidationMap,
									   t[kds_slot_nrooms]));
...
gpupreagg_next_tuple_fallback(GpuPreAggState *gpas, GpuPreAggTask *gpreagg)
{
	...
	for (;;)
	{
		if (gpas->combined_gpujoin)
		{
			...
			/*
			 * MEMO: kds_slot is valid only when GpuPreAgg reported
			 * CpuReCheck error on combined execution mode.
			 * It means a part of rows generated by GpuJoin are already
			 * merged to the final reduction buffer. Thus, we should not
			 * fetch these rows on CPU fallback routine to avoid duplicated
			 * accumulation.
			 */
			if (gpreagg->kds_slot)
			{
				...
				{
					bool   *rows_invalidation_map = (bool *)
						KERN_GPUPREAGG_ROW_INVALIDATION_MAP(&gpreagg->kern);
					if (rows_invalidation_map[row_index])
						continue;
				...
				else if (gpreagg->kgjoin->resume_context)
				{
					/* release kds_slot, then goes to normal fallback path */
					gpuMemFree(gpas->gts.gcontext, (CUdeviceptr)kds_slot);
					gpreagg->kds_slot = NULL;
					gpas->gts.curr_index = 0;

					elog(INFO, "Oops GpuJoin suspended");
				}
				else
				{
					return NULL;
			...
			outer_gts = (GpuTaskState *)outerPlanState(gpas);
		...
		else
		{
			...
			/*
			 * Skip rows if already merged; it is tracked by rows-invalidation-
			 * map if not KDS_FORMAT_BLOCK. In case of KDS_FORMAT_BLOCK,
			 * GpuPreAgg kernel invalidates ItemIdData structure directly.
			 */
			if (pds_src &&
				pds_src->kds.format != KDS_FORMAT_BLOCK &&
				gpas->gts.curr_index < pds_src->kds.nitems)
			{
				bool   *rows_invalidation_map = (bool *)
					KERN_GPUPREAGG_ROW_INVALIDATION_MAP(&gpreagg->kern);
				if (rows_invalidation_map[gpas->gts.curr_index])
				{
					gpas->gts.curr_index++;
					continue;
				}
...
resume_kernel:
	...
	else
	{
			...
			gpreagg->task.kerror.errcode == StromError_CpuReCheck)
		{
			 ...
			 *
			 * Elsewhere, we setup row-invalidation map to inform the rows
			 * to be skipped by CPU fallback.
			 */
			if (pds_src->kds.format == KDS_FORMAT_BLOCK)
			{
			...
			else
			{
				...
				bool	   *rowmap;
				cl_uint		i, sz = sizeof(bool) * pds_src->kds.nitems;

				ri_map = KERN_GPUPREAGG_ROW_INVALIDATION_MAP(&gpreagg->kern);
				rowmap = alloca(sz);
				memset(rowmap, 0, sz);
				for (i=0; i < ri_map->nitems; i++)
				{
					Assert(ri_map->t[i].row_index < pds_src->kds.nitems);
					if (ri_map->t[i].group_id == INT_MAX)
						rowmap[ri_map->t[i].row_index] = true;
				}
				Assert(sz <= gpreagg->kern.row_inval_map_size);
				memcpy(ri_map, rowmap, sz);
			}
		...
		retval = 0;		/* back to the controller */
	}
out_of_resource:
...
resume_kernel:
	...
	if (kgjoin->kerror.errcode != StromError_Success)
	{
		...
		{
			/*
			 * CPU fallback without partial results
			 *
			 * In case when GpuJoin reported CpuReCheck error, obviously,
			 * it didn't touch the final buffer. So, our jobs are simple.
			 * Call the GpuJoin's fallback routine (even though it might
			 * has suspend context).
			 */
			memset(&gpreagg->task.kerror, 0, sizeof(kern_errorbuf));
			gpreagg->task.cpu_fallback = true;

			/* KDS_FORMAT_BLOCK must be written back to the host buffer */
	...
	else if (gpreagg->kern.kerror.errcode != StromError_Success)
	{
			...
			gpreagg->task.kerror.errcode == StromError_CpuReCheck)
		{
			 ...
			 *
			 * In case when GpuPreAgg reported CpuReCheck error, a part of
			 * rows in the kds_slot might be already merged to the final
			 * buffer. CPU fallback code should skip these rows to avoid
			 * duplicated accumulation of rows.
			 */
			gpupreaggRowInvalidationMap *ri_map;
			kern_data_store *kds_slot = (kern_data_store *)m_kds_slot;
			cl_uint		i, sz = sizeof(bool) * kds_slot->nitems;
			bool	   *rowmap;
			int count=0;
			...
			/* makes row-invalidation map for CPU fallback */
			ri_map = KERN_GPUPREAGG_ROW_INVALIDATION_MAP(&gpreagg->kern);
			rowmap = alloca(sz);
			memset(rowmap, 0, sz);
			for (i=0; i < ri_map->nitems; i++)
			{
				Assert(ri_map->t[i].row_index < kds_slot->nitems);
				if (ri_map->t[i].group_id == INT_MAX)
					rowmap[ri_map->t[i].row_index] = true;
				else
				{
			...
			/*
			 * No need to restore GpuJoin's suspend context because
			 * GpuJoin is successfully done and generated kds_slot.
			 * If we have to resume GpuJoin, it should be the next
			 * window from the current suspend context.
			 */
			kgjoin->resume_context = (kgjoin->suspend_count > 0);
