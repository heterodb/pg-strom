...
static void *
pgstrom_opencl_shmem_callback(void *address, Size length)
{
...
pgstrom_opencl_main(Datum main_arg)
{
	...
	pgstrom_setup_shmem(512 << 20, pgstrom_opencl_shmem_callback);
...
pgstrom_init_opencl_server(void)
{
	...
	worker.bgw_restart_time = BGW_NEVER_RESTART; //BGW_DEFAULT_RESTART_INTERVAL;
