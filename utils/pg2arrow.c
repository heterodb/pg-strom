...
pgsql_init_session(PGconn *conn)
{
	 ...
	 *
	 * Note that libpq binary protocol always send the byte stream of
	 * internal data representation; that should be saved in UTC.
	 * So, Arrow files generated by pg2arrow shall have UTC timezone.
	 * It can be enhanced if someone concerned about.
	 */
	query = "show timezone";
	res = PQexec(conn, query);
