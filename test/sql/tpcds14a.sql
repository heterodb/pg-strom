with  cross_items as
 ...
 __cross_items as (
    select distinct ss_item_sk from cross_items
 ),
 avg_sales as
...
(select channel, i_brand_id, i_class_id, i_category_id, sum(sales) sum_sales, sum(number_sales) number_sales
 from (
       select 'store'::text as channel, i_brand_id,i_class_id
       ...
       from store_sales
           ...
           ,__cross_items
           ,avg_sales
       where store_sales.ss_item_sk = __cross_items.ss_item_sk
       ...
       from catalog_sales
           ...
           ,__cross_items
           ,avg_sales
       where catalog_sales.cs_item_sk = __cross_items.ss_item_sk
         and catalog_sales.cs_item_sk = i_item_sk
         and cs_sold_date_sk = d_date_sk
         and d_year = 1998+2 
         and d_moy = 11
       group by i_brand_id,i_class_id,i_category_id,average_sales
       having sum(cs_quantity*cs_list_price) > average_sales
       union all
       ...
       from web_sales
           ...
           ,__cross_items
           ,avg_sales
       where web_sales.ws_item_sk = __cross_items.ss_item_sk
         and web_sales.ws_item_sk = i_item_sk
         and ws_sold_date_sk = d_date_sk
         and d_year = 1998+2
         and d_moy = 11
       group by i_brand_id,i_class_id,i_category_id,average_sales
       having sum(ws_quantity*ws_list_price) > average_sales
 ) y
