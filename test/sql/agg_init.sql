...
DROP TABLE IF EXISTS gpupreagg_test CASCADE;
CREATE TABLE gpupreagg_test (
	id integer,
	key integer,
	smlint_x  smallint,
	integer_x integer,
...
COMMENT ON COLUMN gpupreagg_test.smlint_x  IS '-32768 to 32767';
COMMENT ON COLUMN gpupreagg_test.integer_x IS '-2147483648 to 2147483647';
COMMENT ON COLUMN gpupreagg_test.bigint_x  IS '-9223372036854775808 to 9223372036854775807';
COMMENT ON COLUMN gpupreagg_test.real_x    IS '6 decimal digits precision';
COMMENT ON COLUMN gpupreagg_test.float_x   IS '15 decimal digits precision';
COMMENT ON COLUMN gpupreagg_test.nume_x    IS 'up to 131072 digits before the decimal point; up to 16383 digits after the decimal point';
COMMENT ON COLUMN gpupreagg_test.smlsrl_x  IS '-32768 to 32767';
COMMENT ON COLUMN gpupreagg_test.serial_x  IS '-2147483648 to 2147483647';
COMMENT ON COLUMN gpupreagg_test.bigsrl_x  IS '-9223372036854775808 to 9223372036854775807';
...
INSERT INTO gpupreagg_test SELECT
...
DROP TABLE IF EXISTS gpupreagg_zero_test;
CREATE TABLE gpupreagg_zero_test AS SELECT * FROM gpupreagg_test WITH NO DATA;

--# create overflowed table.
DROP TABLE IF EXISTS gpupreagg_overflow_test;
CREATE TABLE gpupreagg_overflow_test (
	id integer,
	key integer,
	smlint_x  smallint,
	integer_x integer,
...
COMMENT ON COLUMN gpupreagg_overflow_test.smlint_x  IS '-32768 to 32767';
COMMENT ON COLUMN gpupreagg_overflow_test.integer_x IS '-2147483648 to 2147483647';
COMMENT ON COLUMN gpupreagg_overflow_test.bigint_x  IS '-9223372036854775808 to 9223372036854775807';
COMMENT ON COLUMN gpupreagg_overflow_test.real_x    IS '6 decimal digits precision';
COMMENT ON COLUMN gpupreagg_overflow_test.float_x   IS '15 decimal digits precision';
COMMENT ON COLUMN gpupreagg_overflow_test.nume_x    IS 'up to 131072 digits before the decimal point; up to 16383 digits after the decimal point';
COMMENT ON COLUMN gpupreagg_overflow_test.smlsrl_x  IS '-32768 to 32767';
COMMENT ON COLUMN gpupreagg_overflow_test.serial_x  IS '-2147483648 to 2147483647';
COMMENT ON COLUMN gpupreagg_overflow_test.bigsrl_x  IS '-9223372036854775808 to 9223372036854775807';
...
--# [all +random]
--# 1 <= id <= 10000,all columns have positive(+) random numbers.
INSERT INTO gpupreagg_overflow_test SELECT
			generate_series(      1,10000),
			generate_series(  1,10),
...
CREATE UNIQUE INDEX gpupreagg_test_id_unq_idx on gpupreagg_test(id);
CREATE INDEX gpupreagg_test_key_idx on gpupreagg_test(key);

DROP MATERIALIZED VIEW IF EXISTS gpupreagg_mix;
CREATE MATERIALIZED VIEW gpupreagg_mix AS 
SELECT x.id,x.key,
...
FROM
(SELECT * FROM gpupreagg_test WHERE id<=10000) AS x,
(SELECT id-10000 AS id,key-10 AS key,smlint_x,integer_x,bigint_x,real_x,float_x,nume_x,smlsrl_x,serial_x,bigsrl_x FROM gpupreagg_test WHERE key between 11 AND 20) AS y,
(SELECT id-20000 AS id,key-20 AS key,smlint_x,integer_x,bigint_x,real_x,float_x,nume_x,smlsrl_x,serial_x,bigsrl_x FROM gpupreagg_test WHERE key between 21 AND 30) AS z

WHERE x.id=y.id
AND y.id=z.id
AND z.id=x.id;
--# drop key index.
DROP INDEX gpupreagg_test_key_idx;
